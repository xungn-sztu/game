(function () {
  ...
  function addStyle() {
    ...
    style.textContent = `
      ...
      .fv3-send:hover,.fv3-input:hover{filter:brightness(1.12)}
    `;
  ...
  function renderMessage(message) {
    ...
    return mine
      ? `<div class="fv3-row mine">${bubble}${avatar}</div>`
      : `<div class="fv3-row">${avatar}${bubble}</div>`;
  }

  function initShell(shell) {
    if (shell.dataset.fv3Ready === '1') return;
    ...
    root.innerHTML = `
      <div class="fv3-chat">
        <div class="fv3-chat-head">
          ...
          <div class="fv3-sub">${esc(data.platform)}</div>
        </div>
        <div class="fv3-body">${data.messages.map(renderMessage).join('') || '<div class="fv3-system">暂无消息</div>'}</div>
        <div class="fv3-inputbar">
          <button class="fv3-input" data-fill="${esc(hint)}">${esc(hint || '输入消息...')}</button>
